...
# connection = psycopg2.connect("data_sqlite.db")

connection = psycopg2.connect(host=os.environ["HOST"], database=os.environ["DATABASE"], user=os.environ["USER"], password=os.environ["PASSWORD"])
...
def watch_movie(username, title):
    with connection:
        with connection.cursor() as cursor:
            ...
            cursor.execute(INSERT_WATCHED_MOVIE, (username, title))
